...
public class HomePageTests : BusinessCardPageTest
{
    ...
    [Test]
    public async Task HomePage_ShouldShowPrivacyBadge()
    {
        await Page.GotoAsync(BaseUrl);
        await WaitForHomeInteractiveAsync();

        // Check for privacy badge in navbar
        var privacyBadge = Page.Locator("text=Free · No Data Stored");
        await Expect(privacyBadge).ToBeVisibleAsync();
    }
    ...
    public async Task Step1_DownloadTemplateButton_ShouldBeVisible()
    {
        ...
        var downloadButton = Page.Locator("button:has-text('Download Excel Template')");
    ...
    public async Task Step3_GenerateButton_ShouldBeDisabledWithoutTemplate()
    {
        ...
        var generateButton = Page.Locator("button:has-text('Generate')");
        await Expect(generateButton).ToBeDisabledAsync();
    }
    ...
    public async Task ExcelUpload_WithValidFile_ShouldShowSuccessMessage()
    {
        ...
        try
        {
            ...
            // Click upload
            var uploadButton = Page.Locator("button:has-text('Upload Excel')");
