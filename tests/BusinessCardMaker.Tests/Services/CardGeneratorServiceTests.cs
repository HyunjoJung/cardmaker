...
public class CardGeneratorServiceTests
{
    ...
    [Fact]
    public async Task GenerateBatchAsync_ReplacesAllPlaceholders_InRealTemplate()
    {
        // Arrange: use the tracked production template
        ...
        var employee = new Employee
        {
            ...
            Position = "Manager",
            PositionEnglish = "Manager",
            Email = "jane@cardmaker.test",
            Mobile = "010-1234-5678",
            Extension = "1234",
            Company = "CardMaker Inc.",
            Department = "R&D"
        };
        ...
        // Assert
        Assert.True(result.Success, string.Join(";", result.Errors ?? new List<string>()));
        using var pptStream = await ExtractSinglePresentationAsync(result.ZipFilePath!);
        using var ppt = PresentationDocument.Open(pptStream, false);
        var slideParts = ppt.PresentationPart!.SlideParts.ToList();
        var texts = slideParts
            .SelectMany(sp => sp.Slide.Descendants<A.Text>())
            .Select(t => t.Text)
            .ToList();

        // Required values present
        Assert.Contains(texts, t => t.Contains("Jane Doe", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(texts, t => t.Contains("Manager", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(texts, t => t.Contains("jane@cardmaker.test", StringComparison.OrdinalIgnoreCase));
        Assert.Contains(texts, t => t.Contains("1234", StringComparison.OrdinalIgnoreCase)); // extension

        // Placeholders removed
        var placeholders = new[] { "{name}", "{name_en}", "{position}", "{position_en}", "{email}", "{mobile}", "{extension}", "{qrcode}" };
        foreach (var placeholder in placeholders)
        {
            Assert.False(texts.Any(t => t.Contains(placeholder, StringComparison.OrdinalIgnoreCase)), $"Placeholder '{placeholder}' should be replaced.");
        }

        // QR code rendered as image
        var pictures = slideParts.SelectMany(sp => sp.Slide.Descendants<P.Picture>()).Count();
        Assert.True(pictures > 0, "Expected at least one QR code image on the slide.");
    }
    ...
    private static string ResolveTemplatePath()
    {
        var path = Path.GetFullPath(
            Path.Combine(AppContext.BaseDirectory,
                "../../../../../src/BusinessCardMaker.Web/wwwroot/templates/CardMaker_Template.pptx"));

        if (!File.Exists(path))
        {
            throw new FileNotFoundException("CardMaker_Template.pptx not found for tests.", path);
        }
