...
// Ensure PPT template is served even if static web assets are skipped in some hosts
app.MapGet("/templates/CardMaker_Template.pptx", (IWebHostEnvironment env) =>
{
    var path = Path.Combine(env.WebRootPath, "templates", "CardMaker_Template.pptx");
    if (!File.Exists(path))
    {
        return Results.NotFound();
    }

    return Results.File(path, "application/vnd.openxmlformats-officedocument.presentationml.presentation", "CardMaker_Template.pptx");
}).WithName("DownloadCardMakerTemplate").DisableAntiforgery();
